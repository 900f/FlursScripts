...
export default async function handler(req, res) {
  ...
  try {
      ...
      await sql`
        ...
        ON CONFLICT (hash) DO UPDATE
          SET content = EXCLUDED.content,
              label   = COALESCE(NULLIF(${label || ''}, ''), scripts.label)
      `;
    ...
    if (action === 'get') {
      ...
      const row = rows[0];
      return res.status(200).json({ ok: true, hash: row.hash, label: row.label, content: row.content });
    }

    // ── RENAME ────────────────────────────────────────────────────────────
    if (action === 'rename') {
      if (!hash || !label) return res.status(400).json({ error: 'Missing hash or label' });
      const result = await sql`UPDATE scripts SET label = ${label.trim()} WHERE hash = ${hash}`;
      if (result.count === 0) return res.status(404).json({ error: 'Script not found' });
      return res.status(200).json({ ok: true });
    }

    // ── LIST ──────────────────────────────────────────────────────────────
    if (action === 'list') {
      const rows = await sql`SELECT hash, label, created_at FROM scripts ORDER BY created_at DESC`;
      const scripts = rows.map(r => ({ hash: r.hash, label: r.label, created: r.created_at }));
