...
const { action, password, hash, label, content, loaderUrl } = req.body || {};
...
// ── SAVE LOADER (bootstrap loader from URL) ───────────────────────────
if (action === 'saveloader') {
  if (!loaderUrl) return res.status(400).json({ error: 'Missing loaderUrl' });

  // Validate it looks like a URL
  if (!loaderUrl.startsWith('http://') && !loaderUrl.startsWith('https://')) {
    return res.status(400).json({ error: 'loaderUrl must start with http:// or https://' });
  }

  const newHash = hash || generateHash();

  // Generate the loader Lua — just a tiny stub that fetches the real script
  const loaderContent = `loadstring(game:HttpGet("${loaderUrl}",true))()`;

  await put(`scripts/${newHash}.lua`, loaderContent, {
    access: 'public', contentType: 'text/plain', addRandomSuffix: false,
  });

  await saveMeta(newHash, {
    hash:      newHash,
    label:     label || 'Loader → ' + loaderUrl,
    created:   Date.now(),
    isLoader:  true,
    loaderUrl: loaderUrl,
  });

  return res.status(200).json({ ok: true, hash: newHash });
}
