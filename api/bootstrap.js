...
// Works whether Vercel parsed the body or not
...
function xorEncode(text, seed) {
    const bytes = Buffer.from(text, 'utf8'), out = [];
    let state = seed >>> 0;
...
function buildLua(meta) {
    ...
    const seedHi = (seed >>> 16) & 0xFFFF, seedLo = seed & 0xFFFF;
    const vData=randVar(), vSeed=randVar(), vState=randVar(),
          vOut=randVar(),  vI=randVar(),   vDec=randVar(),
          vFn=randVar(),   vErr=randVar();
...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        ...
        } catch (err) { console.error(err); return res.status(500).end('-- error'); }
    }
    ...
    try {
        if (action === 'generate') {
            let content, mode;
            if (rawLua?.trim())      { content = rawLua.trim();      mode = 'lua'; }
