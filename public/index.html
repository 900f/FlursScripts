...
<body>
    ...
    (function() {
        ...
        function getLoaderSnippet(hash) {
            return `script_key="YOUR-KEY"\nloadstring(game:HttpGet("https://api.flurs.xyz/files/v3/loader/${hash}.lua"))()`;
        }
        ...
        async function renderKeyScripts() {
            ...
            container.innerHTML = scripts.map(function(s) {
                return '<div class="dash-script-row" id="ks-row-' + s.hash + '">' +
                    ...
                    '</div></div>' +
                    '<div class="dash-script-row-actions">' +
                    '<button class="dash-icon-btn" title="Copy hash" data-ks-copy="' + escHtml(s.hash) + '"><i class="fa-solid fa-copy"></i></button>' +
            ...
            container.querySelectorAll('[data-ks-copy]').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    navigator.clipboard.writeText(btn.getAttribute('data-ks-copy')).catch(()=>{});
